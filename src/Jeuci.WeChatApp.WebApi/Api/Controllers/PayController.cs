...
using Jeuci.WeChatApp.Recharge;
...
namespace Jeuci.WeChatApp.Api.Controllers
{
    public class PayController : AbpApiController
    {
        ...
        public async Task<HttpResponseMessage> RechargeService()
        {
            LogHelper.Logger.Debug("开始回调充值接口");
            Senparc.Weixin.MP.TenPayLibV3.ResponseHandler payNotifyRepHandler =
                new Senparc.Weixin.MP.TenPayLibV3.ResponseHandler(HttpContext.Current);
            payNotifyRepHandler.SetKey(WxPayConfig.KEY);
            string return_code = payNotifyRepHandler.GetParameter("return_code"); //返回状态码
            ...
            var result = _rechargeAppService.CompleteRechargeOrder(payData);
            if (result.Code == ResultCode.Success)
            {
                xml = "<xml>" +
                      "<return_code><![CDATA[SUCCESS]]></return_code>" +
                ...
                LogHelper.Logger.Debug(result.Msg);
            }
            else
            {
                xml = "<xml>" +
