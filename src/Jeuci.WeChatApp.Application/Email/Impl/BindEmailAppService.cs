...
namespace Jeuci.WeChatApp.DirectEmail.Impl
{
    public class BindEmailAppService : IBindEmailAppService
    {
        ...
        {
            var result =await _bindEmailProcessor.SendValidByEmial(openId,emailAddress);
            if (result)
            {
                return new ResultMessage<string>("OK");
            }
            return new ResultMessage<string>(ResultCode.Fail,"失败");
        }
    }
}